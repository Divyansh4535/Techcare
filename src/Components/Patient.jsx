...
const Patient = () => {
    const { patientData, setPatientDetails } = useContext(UserContext)
    ...
    return (
        <div className=' w-full  rounded-[15px] flex flex-col items-center h-full justify-between p-2 '>
            ...
            <div className='w-full h-full overflow-y-auto scrollbar flex flex-col items-center justify-start gap-3 pt-4  mt-3'>
                {patientData?.map((data, idx) => (
                        ...
                        >
                            <div className='flex items-center gap-1'>
                                <img src={data.profile_picture} alt={data.name} className='size-10' />
                                <div className='flex items-start  justify-start leading-1 flex-col px-3'>
                                    <p className='text-[14px] font-bold '>{data.name}</p>
                                    <p className='text-[#707070] text-[12px] font-normal'> {data.gender} ,{data.age}  </p>
                                </div>
                            </div>
                            <span>
                                <svg xmlns="http://www.w3.org/2000/svg" width="12" height="3.714" viewBox="0 0 18 3.714">
